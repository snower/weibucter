...
*/
                $full_source_path = preg_replace('/\/+|\\\+/','/',$this->source_image);
                $x = explode('/', $full_source_path);
                $this->source_image = end($x);
                $this->source_folder = str_replace($this->source_image, '', $full_source_path);
 ...
 */
	...
	function image_create_gd($path = '', $image_type = '')
	{
       ...
       //--SAE--         
      $path = s_get_wrappers_bypath($path);
	...
	function image_save_gd($resource)
	{
		...
		switch ($this->image_type)
		{
			case 1 :
						...
						if ( ! @imagegif($resource, SAE_TMP_PATH.'dst'))
						{
			...
			case 2	:
						...
						if ( ! @imagejpeg($resource,  'dst', $this->quality))
						{
							$this->set_error('imglib_save_failed');
							return FALSE;
						}
			...
			case 3	:
						...
						if ( ! @imagepng($resource,  SAE_TMP_PATH.'dst'))
						{
							$this->set_error('imglib_save_failed');
							return FALSE;
						}
			...
			default		:
                ...
                s_writefile($this->full_dst_path, file_get_contents(SAE_TMP_PATH.'dst' ) );
		return TRUE;
	}
...
//--SAE-- 
                $vals = @getimagesize(s_get_wrappers_bypath($path) );
