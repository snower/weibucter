 ...
 */
                $CI =& get_instance();
                $cache_adapter;
                $cach_model = $CI->config->item('sea_output_cache');
                ...
                switch ($cach_model){
                    ...
                    case 'memcache':
                        ...
                        $cache_key = ($path == '') ? 'system_cache_' : $path;
                        break;
                }
 ...
 */
                $cache_adapter;
                $cach_model = $CFG->item('sea_output_cache');
                ...
                switch ($cach_model){
                    ...
                    case 'vkdb':
                        ...
                        break;
                    case 'memcache':
                        $status = memcache_init();
                        $cache_adapter =  new Memcache();
                        break;
                }
               if( !$status){
                    return FALSE;
		...
		$cache_key .= md5($uri);
                ...
                if( $cach_model == 'memcache'  ){
                    if(is_array($cache)){ 
                        $this->_display( $cache[0] );
                        return TRUE;
                    }else{
                       return FALSE; 
                    }
                    ...
                    if (time() > $cache['time'] + $cache['ttl']) {
                        $cache_adapter->delete( $cache_key );
                        return FALSE;
                    }
                    $this->_display($cache['data']);
                    return TRUE;
                }
	}
